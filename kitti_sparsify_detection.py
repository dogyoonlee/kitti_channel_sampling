...
def pto_ang_map(velo_points,
    ...
    d[d == 0] = 0.000001
    r[r == 0] = 0.000001
    phi = np.radians(45.) - np.arcsin(
        y / r)  # 45 - angle from x axis to y axis
...
def gen_sparse_points_seq(lidar_path, outputfolder, seq):
    ...
    for data_idx in tqdm.tqdm(data_idx_list, desc="sequence " + seq):
        sparse_points = gen_sparse_points(os.path.join(lidar_path, data_idx),
                                          args)
...
def gen_sparse_points_all(args):
    if args.data_mode == 'training':
        lidar_path = os.path.join(os.path.join(args.data_dir, args.data_mode),
                                  'velodyne')
    else:
        lidar_path = os.path.join(
            os.path.join(os.path.join(args.data_dir, args.data_mode),
                         'velodyne'), 'velodyne')
    if args.multi_lidar:
        outputfolder = os.path.join(
            os.path.join(
                os.path.join(args.sparse_lidar_save_path,
                             str(int(64 / int(args.slice))) + 'channel_X2'),
                args.data_mode), 'velodyne')
    else:
        outputfolder = os.path.join(
            os.path.join(
                os.path.join(args.sparse_lidar_save_path,
...
def gen_sparse_points_for_det(lidar_path, outputfolder):
    # print('lidar_path: ', lidar_path)
    # print('os.listdir(lidar_path): ', os.listdir(lidar_path))
    # exit()
    data_idx_list = sorted(
        [x.strip() for x in os.listdir(lidar_path) if x[-3:] == 'bin'])

    for data_idx in tqdm.tqdm(data_idx_list, desc="det"):
        sparse_points = gen_sparse_points(os.path.join(lidar_path, data_idx),
                                          args)
        # print('processed pc_velo: ', sparse_points)
        # print('processed pc_velo shape: ', sparse_points.shape)
...
def gen_sparse_points_det(args):
    if args.data_mode == 'training':
        lidar_path = os.path.join(os.path.join(args.data_dir, args.data_mode),
                                  'velodyne')
    else:
    ...
    # outputfolder = args.sparse_lidar_save_path
    os.makedirs(outputfolder, exist_ok=True)
    gen_sparse_points_for_det(lidar_path, outputfolder)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sparse_lidar_save_path',
    ...
    parser.add_argument('--det', action='store_true', help='detection, no seq')
