...
class kitti_object(object):
    ...
    def __init__(self, root_dir, args=None):
        ...
        self.split_dir = os.path.join(root_dir, self.split) # args.dir/split('training'/'testing')
        ...
        self.depth_dir = os.path.join(os.path.join(self.split_dir, depth_dir), seq_id)
        
        # self.pred_dir  = os.path.join(self.split_dir, pred_dir)
        self.pred_dir_list  = [os.path.join(os.path.join(args.preddir,'car_3d_det_val_thres'),('trk_id_included_dy_st_term_1_thres_')+thres_car),
                               os.path.join(os.path.join(args.preddir,'ped_3d_det_val_thres'),('trk_id_included_dy_st_term_1_thres_')+thres_ped),
                               os.path.join(os.path.join(args.preddir,'cyc_3d_det_val_thres'),('trk_id_included_dy_st_term_1_thres_')+thres_cyc)]
        
        if os.path.exists(self.pred_dir_list[0]) is False:
            self.pred_dir_list[0] = os.path.join(os.path.join(args.preddir,'car_3d_det_val_thres'),('trk_id_included_dy_st_term_1_thres_')+thres_car_default)
        if os.path.exists(self.pred_dir_list[1]) is False:
            self.pred_dir_list[1] = os.path.join(os.path.join(args.preddir,'car_3d_det_val_thres'),('trk_id_included_dy_st_term_1_thres_')+thres_ped_default)
        if os.path.exists(self.pred_dir_list[2]) is False:
            self.pred_dir_list[2] = os.path.join(os.path.join(args.preddir,'car_3d_det_val_thres'),('trk_id_included_dy_st_term_1_thres_')+thres_cyc_default)
        
        self.pred_dir_list_seq  = [os.path.join(self.pred_dir_list[0], seq_id),
    ...
    def get_pred_objects(self, idx, seq_id, det_type=0):
        assert(idx<self.num_samples and self.split=='training')
        pred_filename = os.path.join(self.pred_dir_list_seq[det_type], '%06d.txt'%(idx))
        is_exist = os.path.exists(pred_filename)
        if is_exist:
            return utils.read_label(pred_filename)
        else:
...
def show_image_with_3D_boxes(img, objects, objects_pred, objects_occ, calib, show3d=True, depth=None):
    ''' Show image with 2D bounding boxes '''
    img1 = np.copy(img) # for 2d bbox
    img2 = np.copy(img) # for 3d bbox
    img3 = np.copy(img) # for 3d bbox
...
def get_lidar_in_image_fov(pc_velo, calib, xmin, ymin, xmax, ymax,
                           return_more=False, clip_distance=2):
...
def show_lidar_with_depth(pc_velo,
    ...
    if objects_pred_car is not None:
        ...
        for obj in objects_pred_car:
            ...
            draw_gt_boxes3d([box3d_pts_3d_velo], fig=fig, color=color, draw_text=True, score=obj.score, obj_id=obj_id, state=obj_state, det_class=obj.type)
            fg_seg([box3d_pts_3d_velo], pc_velo, fig, color)
            # Draw heading arrow
    ...
    mlab.view(azimuth=azimuth,elevation=elevation,focalpoint=focalpoint,distance=distance)
...
def show_segmented_lidar_with_depth(pc_velo,
    ...
    if objects_pred is not None:
        ...
        for obj in objects_pred:
            ...
            print("box3d_pts_3d_velo:")
            print(box3d_pts_3d_velo)
            draw_gt_boxes3d([box3d_pts_3d_velo], fig=fig, color=color, draw_text=True, score=obj.score)
            # Draw heading arrow
            ori3d_pts_2d, ori3d_pts_3d = utils.compute_orientation_3d(obj, calib.P)
            ori3d_pts_3d_velo = calib.project_rect_to_velo(ori3d_pts_3d)
            x1,y1,z1 = ori3d_pts_3d_velo[0,:]
...
def show_occluded_lidar_with_depth(pc_velo,
    ...
    print(('All point num: ', pc_velo.shape[0]))
    fig = mlab.figure(figure=None, bgcolor=(0.1, 0.1, 0.1),
        fgcolor=None, engine=None, size=(1000, 500))
    if img_fov:
        pc_velo_index = get_lidar_index_in_image_fov(pc_velo[:,:3], calib, 0, 0,
            img_width, img_height)
    ...
    if objects_pred is not None:
        ...
        for obj in objects_pred:
            ...
            # print("box3d_pts_3d_velo:")
            # print(box3d_pts_3d_velo)
            draw_gt_boxes3d([box3d_pts_3d_velo], fig=fig, color=color, draw_text=False, score=obj.score)
            fg_seg([box3d_pts_3d_velo], pc_occ, fig, color)
            # Draw heading arrow
            ori3d_pts_2d, ori3d_pts_3d = utils.compute_orientation_3d(obj, calib.P)
            ori3d_pts_3d_velo = calib.project_rect_to_velo(ori3d_pts_3d)
...
def show_lidar_on_image_with_box(pc_velo, img, calib, img_width, img_height, objects, show3d=True, depth=None, objects_pred=None):
    ''' Project LiDAR points to image '''
    imgfov_pc_velo, pts_2d, fov_inds = get_lidar_in_image_fov(pc_velo,
                                                              calib, 0, 0, img_width, img_height, True)
    imgfov_pts_2d = pts_2d[fov_inds, :]
...
def dataset_viz_save(root_dir, args): # args.dir, args
    ...
    MM_PER_VIEW = 180, 40, [24, -1.04700089, -2.03249991], 120.0 # azimuth, elevation, focalpoint, distance

    ## load 2d detection results
    # objects2ds = read_det_file('box2d.list')
    cnt = 0
    figure_bg = mlab.figure(figure=None, bgcolor=(0,0,0),
    ...
    for data_idx in range(args.ii, len(dataset)):
        ...
        if args.pred:
            ...
            #    continue
            objects_pred_car = dataset.get_pred_objects(data_idx, args.seq, det_type=0)
            objects_pred_ped = dataset.get_pred_objects(data_idx, args.seq, det_type=1)
            objects_pred_cyc = dataset.get_pred_objects(data_idx, args.seq, det_type=2)
        ...
        lidar_filename_tmp = os.path.join(os.path.join(os.path.join(os.path.join(root_dir,args.split),args.lidar),args.seq),'%06d.bin'%(data_idx))
...
if __name__=='__main__':
    ...
    parser.add_argument('-d', '--dir', type=str, default="/SSD/Dataset/kitti_t_o", metavar='N',
                        help='input  (default: data/object)')
    parser.add_argument('-i', '--ind', type=int, default=0, metavar='N',
                        help='input index')
    parser.add_argument('-ii', type=int, default=0, metavar='N', help='starting input index')
    ...
    parser.add_argument('-sf','--save_fig', action='store_true', help='show predict results')
    ...
    parser.add_argument('-sp','--split', type=str, default="training", metavar='N',
    ...
    parser.add_argument('-r','--preddir', type=str, default="./results", metavar='N',
                        help='predicted boxes  (default: pred)')
    parser.add_argument('-sq','--seq', type=str, default="0000", metavar='N',
                        help='seq  (default: 0000)')
    parser.add_argument('-th_c','--thrs_car', type=str, default="0.66", metavar='N', help='predicted boxes  (default: threshold)')
    parser.add_argument('-th_p','--thrs_ped', type=str, default="0.2", metavar='N', help='thrs_ped  (default: threshold)')
    parser.add_argument('-th_cl','--thrs_cyc', type=str, default="0.15", metavar='N', help='thrs_cyc  (default: threshold)')
    parser.add_argument('--occdir', type=str, default="PR_OUT/0.82_98", metavar='N',
                        help='occ_dir  (default: occ_pred)')
    parser.add_argument('--occpointdir', type=str, default="PR_OUT/occluded_points", metavar='N',
    ...
    parser.add_argument('--show_image_with_3D_boxes', action='store_true', help='show lidar')
    parser.add_argument('--show_lidar_topview_with_boxes', action='store_true', help='show lidar topview')
    parser.add_argument('--show_lidar_on_image_with_box', action='store_true', help='show lidar on image with box')
    parser.add_argument('--show_segmented_lidar_with_depth', action='store_true', help='show segmented lidar')
    parser.add_argument('--show_occluded_lidar_with_depth', action='store_true', help='show occluded lidar')
