...
def draw_lidar(
    ...
    mlab.text3d(
        0, 2.5, -0.1, "LiDAR", scale=(0.4, 0.4, 0.4), color=(1, 1, 1), figure=fig
    )
...
def fg_seg(box, pc, fig, color):
    ...
    for n in range(pc.shape[0]):
        ...
        if (
            0 < v.dot(i) < i.dot(i)
            and 0 < v.dot(j) < j.dot(j)
            and 0 < v.dot(k) < k.dot(k)
        ):
            fg[n] = 1
    fore_index = fg == 1
    if color != (0, 0, 0):
    ...
    for n in range(num):
        ...
        if draw_text:
            mlab.text3d(
                ...
                b[4, 1],
                b[4, 2] + 0.2,
                "%.2f" % torch.sigmoid(torch.tensor(score)),
                scale=text_scale,
                color=color,
                figure=fig,
            )
